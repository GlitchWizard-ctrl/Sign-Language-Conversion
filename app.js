...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function handleTabSwitch(tabName) {
        ...
        if (tabName === "recordTab") {
            ...
            activeMode = "predict";
            fetchModelStatus();
        } else if (tabName === "callTab") {
            activeMode = "call";
            // Intentionally do NOT stop an active call when switching tabs —
            // the call should keep running in the background like a real meeting.
        }
    }
    ...
    // ==========================================================================
    // 4. GROUP CALL TAB (Jitsi Meet — supports many simultaneous participants)
    // ==========================================================================
    const callRoomNameInput = document.getElementById("callRoomName");
    ...
    // Give every operator a stable-ish default room name they can share/edit
    function defaultRoomName() {
        const clean = (localStorage.getItem("username") || "isl-team")
            .toLowerCase()
            .replace(/[^a-z0-9]+/g, "-");
        return `isl-${clean}-room`;
    }
    callRoomNameInput.value = defaultRoomName();

    function setParticipantCount(n) {
        callParticipantCount.textContent = n;
    }

    function startOrJoinCall() {
        if (jitsiApi) return; // already in a call

        const roomName = (callRoomNameInput.value || defaultRoomName()).trim();
        if (!roomName) {
            showToast("Please enter a room name to start the call.", "error");
            return;
        }

        if (typeof JitsiMeetExternalAPI === "undefined") {
            showToast("Video call engine failed to load. Check your connection.", "error");
            return;
        }
        ...
        const displayName = localStorage.getItem("username") || "Operator";
        ...
        jitsiApi = new JitsiMeetExternalAPI("meet.jit.si", {
            ...
            interfaceConfigOverwrite: {
                TOOLBAR_BUTTONS: [
                    ...
                    "tileview", "hangup", "fullscreen", "participants-pane"
                ],
        ...
        jitsiApi.addEventListener("videoConferenceJoined", () => {
            showToast(`Joined call room "${roomName}"`, "success");
            setParticipantCount(1);
        });

        jitsiApi.addEventListener("participantJoined", () => {
            jitsiApi.getParticipantsInfo().then(list => setParticipantCount(list.length + 1));
            showToast("A participant joined the call.", "info");
        });
        ...
        leaveCallBtn.classList.remove("btn-disabled");
        leaveCallBtn.disabled = false;
        callRoomNameInput.disabled = true;
    }
    ...
    function teardownCall() {
        ...
        leaveCallBtn.classList.add("btn-disabled");
        leaveCallBtn.disabled = true;
        callRoomNameInput.disabled = false;
    }

    startCallBtn.addEventListener("click", startOrJoinCall);
